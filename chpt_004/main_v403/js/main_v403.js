...
// commands f=forward, b=backward, a=attack (fire), r=rotate, 
// 1 unit of movement is half height of tank
var tankCommandList = [];
...
//	for (var i = 0; i < tankCommandList.length;i++)
//	{
	if (tankCommandList.length > 0)
	{
		switch (tankCommandList[0])
		{
			case 'f':
			{
				mainTank.y -= 13;
		...
		draw();
		tankCommandList.shift();
		if (tankCommandList.length > 0){
			setTimeout(runCommands,400);
		}
	}
//	}
}
...
function rotateTank(evt)
{
	...
	switch (evt.keyCode)
	{
		...
		case 38: //up
		{
			tankCommandList.push('f');
			break;
		}
		case 39 : //right
		{
			tankCommandList.push('r');
			//draw();
			break;
		}
		case 40: //down
		{
			tankCommandList.push('b');
			break;
		}
		case 71: // g 
		{
